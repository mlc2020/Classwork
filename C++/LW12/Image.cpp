...
Image::Image(const Image& oldImage){
    ...
    height = 0;
    filename = "";
    this->filename = oldImage.filename;
    allocateImage(oldImage.width, oldImage.height);

    for (unsigned int i = 0; i < oldImage.height; i++){
        for (unsigned int j = 0; j < oldImage.width; j++){
            image[j][i].setRed(oldImage.image[j][i].getRed());
            image[j][i].setGreen(oldImage.image[j][i].getGreen());
            image[j][i].setBlue(oldImage.image[j][i].getBlue());
        }
    }
}
