...
void grayscaleImage(Pixel image[MAX_WIDTH][MAX_HEIGHT], int width, int height) {
  ...
  int grayscale = 0;
     for(int j = 0; j < height; ++j) {
         for(int i = 0; i < width; ++i) {
        grayscale = round((image[i][j].r + image[i][j].g + image[i][j].b)/3.0);
...
void sepiaImage(Pixel image[MAX_WIDTH][MAX_HEIGHT], int width, int height) {
  ...
  for(int j = 0; j < height; ++j) {
      for(int i = 0; i < width; ++i) {
          ...
          oldblue = image[i][j].b;
          image[i][j].r = round((.393*oldred) + (.769*oldgreen) + (.189*oldblue));
          image[i][j].g = round((.349*oldred) + (.686*oldgreen) + (.168*oldblue));
          image[i][j].b = round((.272*oldred) + (.534*oldgreen) + (.131*oldblue));
          if (image[i][j].r > 255) {
              image[i][j].r = 255;
          } if (image[i][j].g > 255) {
              image[i][j].g = 255;
